...
// but is much less computationally intensive---it can be performed on a 3.3 V Pro Mini operating at 8 MHz!
        void MPU9265::MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
        {
...
void MPU9265::readAccelData(int16_t * destination)
{
...
void MPU9265::readGyroData(int16_t * destination)
{
...
void MPU9265::readMagData(int16_t * destination)
{
  ...
  if(readByte(MAG_ST1) & 0x01) { // wait for magnetometer data ready bit to be set
  readBytes(MAG_XOUT_L, 7, &rawData[0]);  // Read the six raw data and ST2 registers sequentially into data array
...
void MPU9265::initMAG(float * destination)
{
  // First extract the factory calibration for each magnetometer axis
  uint8_t rawData[3];  // x/y/z gyro calibration data stored here
  address = MAG_ADDRESS;
  writeByte(MAG_CNTL, 0x00); // Power down magnetometer  
  delay(10);
  writeByte(MAG_CNTL, 0x0F); // Enter Fuse ROM access mode
  delay(10);
  readBytes(MAG_ASAX, 3, &rawData[0]);  // Read the x-, y-, and z-axis calibration values
  ...
  writeByte(MAG_CNTL, Mscale << 4 | Mmode); // Set magnetometer data resolution and sample ODR
  delay(10);
  address = MPU9250_ADDRESS;
}

void MPU9265::initMPU9250()
{  
 // wake up device
